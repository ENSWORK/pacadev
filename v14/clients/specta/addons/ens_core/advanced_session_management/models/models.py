...
class BaseModel(models.AbstractModel):
    ...
    def write(self, vals):
        try:
            ...
            if bool(request) and self._name not in exclude_models.ignore_model and moduel_state == 'installed':
    ...
    def create(self, vals_list):
        ...
        for record in res:
            if bool(request) and record._name not in exclude_models.ignore_model and moduel_state == 'installed':
                activity_log_obj = self.env['activity.log'].sudo()
...
class ir_module_module(models.Model):
    ...
    def _button_immediate_function(self, function):
        ...
        for record in self:
            if bool(request) and record.name != 'advanced_session_management':
                login_log = record.env['login.log'].sudo().search([('session_id','=',request.session.sid)],limit=1)
                if login_log:
                    value = ''
