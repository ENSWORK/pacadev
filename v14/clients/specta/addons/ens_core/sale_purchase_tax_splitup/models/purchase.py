...
#Added By L --> Compute tax amount by group
    """
        Note: we can just loop inside purchase_tax_line_ids and print Goups of tax with thers amount, 
        cause they are already grouped, BUT the TVA 20% immobilisation is not grouped with TVA 20% so we need this function to group theme all.
        the function return a amount_by_group of type dictionary ex: amount_by_group{"group1":"amount1"}
    """
    @api.onchange('order_line')
    def _amount_by_group(self):
        for order in self:
            groups = []
            #Collect DISTINC Groups IDs from purchase order lines
            for line in order.purchase_tax_line_ids:
                group = line.tax_group_name.id
                if group not in groups:
                    groups.append(group)

            # Compute amounts by Groups
            amount_by_group = {}
            for i in groups:
                #amount_by_group.update({i: 'amount'})
                amount = 0.0
                group = ''
                for line in order.purchase_tax_line_ids:
                    if line.tax_group_name.id == i:
                        amount += line.amount
                        group = line.tax_group_name.name
                amount_by_group.update({group: amount})
...
#                     raise Warning('Tax Base Amount not computable probably due to a change in an underlying tax (%s).', tax.tax_id.name)
    ...
    #ADD
    tax_group_name = fields.Many2one('account.tax.group', string='Groupe', related='tax_id.tax_group_id', store=True, readonly=False)
