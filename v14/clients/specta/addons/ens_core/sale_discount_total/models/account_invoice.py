...
class AccountInvoice(models.Model):
    ...
    #@api.onchange('discount_type', 'discount_rate', 'invoice_line_ids') --Original Line
    @api.onchange('discount_type', 'discount_rate')      #  MODD BY L to save manual discount
...
class AccountInvoiceLine(models.Model):
    ...
    @api.onchange('product_id')      #  ADD BY L set discount
    def set_disc(self):
        for invoiceline in self:
            if invoiceline.move_id.discount_type == 'percent':
                for line in invoiceline:
                    line.discount = invoiceline.move_id.discount_rate
            else:
                total = discount = 0.0
                for line in invoiceline:
                    total += round((line.quantity * line.price_unit))
                if invoiceline.move_id.discount_rate != 0 and total != 0:
                    discount = (invoiceline.move_id.discount_rate / total) * 100
                else:
                    discount = invoiceline.move_id.discount_rate
                for line in invoiceline:
                    line.discount = discount
