...
class StockPicking(models.Model):
    ...
    #test2 = fields.Boolean('test2',default=False)
    ...
    def get_payment(self):
        for r in self:
            if r.state == 'posted' and r.payment_state == 'paid':
                payments = self.env['account.payment'].sudo().search([('ref', '=', r.payment_reference)])
                if payments:
